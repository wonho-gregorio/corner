...
이 문서는 확정된 1차 MVP 정책을 PostgreSQL 17 관계형 모델로 옮긴 기준입니다. 전체 설계는 완료되었고 `auth`·`member` 영역은 **V1**, 상품·행사·회원권 영역은 **V2**, 청구·결제·환불·회원권 해지 영역은 **V3 구현 완료** 상태입니다.
...
V1~V3은 Flyway 스키마와 JPA 영속 엔티티가 함께 구현되어 있습니다. 애플리케이션 서비스와 HTTP API는 후속 구현 범위입니다.
